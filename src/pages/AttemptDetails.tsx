...
import type { AttemptDetailsResponseDto } from '../utils/types';
import { IMAGE_BASE_URL } from '../utils/constants';

export const AttemptDetails: React.FC = () => {
  const { attemptId } = useParams<{ attemptId: string }>();
  const navigate = useNavigate();
  const [data, setData] = useState<AttemptDetailsResponseDto | null>(null);
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      try {
        setLoading(true);
        setError(null);
        const response = await mockService.fetchAttemptDetails(attemptId);
        setData(response);
        ...
        setError(err.response?.data?.message || 'Failed to load attempt details. Please try again.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const progressColor = data?.percentage
    ? data.percentage >= 70
      ? 'bg-green-500'
      : data.percentage >= 50
      ? 'bg-yellow-500'
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <p className="text-red-500 text-lg mb-4">{error || 'Attempt details not found'}</p>
          <Button onClick={() => navigate('/my-attempts')}>Back to My Attempts</Button>
        </div>
      </div>
    );
  }
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <div className="flex gap-4 text-sm text-gray-500 mt-1">
                <span>Score: {data.obtainedMarks.toFixed(2)} / {data.totalMarks}</span>
                <span className={getPercentageColor(data.percentage || null)}>
                  {data.percentage?.toFixed(2) || 'N/A'}%
                </span>
              </div>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
            <div className="text-center">
              ...
              <p className="text-lg font-semibold">{data.totalQuestions}</p>
            </div>
            <div className="text-center">
              <span className="text-sm text-gray-500">Answered</span>
              <p className="text-lg font-semibold">{data.answeredQuestions}</p>
            </div>
            <div className="text-center">
              <span className="text-sm text-green-600">Correct</span>
              <p className="text-lg font-semibold text-green-600">{data.correctAnswers}</p>
            </div>
            <div className="text-center">
              <span className="text-sm text-red-600">Incorrect</span>
              <p className="text-lg font-semibold text-red-600">{data.incorrectAnswers}</p>
            </div>
            <div className="text-center">
              <span className="text-sm text-gray-500">Unanswered</span>
              <p className="text-lg font-semibold">{data.unansweredQuestions}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {data.sectionWiseResults.map((sectionResult) => (
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-start gap-2">
                                <span className={`text-sm font-semibold min-w-[20px] flex-shrink-0 ${textColor}`}>
                                  {option.label}.
                                </span>
                                <div className="flex-1 min-w-0">
                                  {option.text && (
                                    <span className={`text-sm leading-relaxed whitespace-pre-line ${
                                      isUserSelected && isCorrect ? 'font-bold' : ''
                                    } ${textColor}`}>
                                      {option.text}
                                    </span>
                                  )}
                                ...
                                {isUserSelected && !isCorrect && (
                                  <span className="text-red-600 font-semibold text-xs flex-shrink-0 ml-2">
                                    ✗
                                  </span>
                                )}
                              </div>
